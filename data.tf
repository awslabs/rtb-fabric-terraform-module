...
# ASG Service Discovery Role validation data sources (only when role is provided and manual setup)
data "aws_iam_role" "asg_service_discovery_role" {
  ...
  name  = var.responder_gateway.managed_endpoint_configuration.auto_scaling_groups_configuration.asg_discovery_role
}

# Validate ASG service discovery role trust policy contains RTB Fabric service principals
locals {
  asg_service_discovery_role_trust_policy = var.responder_gateway.create && var.responder_gateway.managed_endpoint_configuration != null && var.responder_gateway.managed_endpoint_configuration.auto_scaling_groups_configuration != null && var.responder_gateway.managed_endpoint_configuration.auto_scaling_groups_configuration.asg_discovery_role != null ? jsondecode(data.aws_iam_role.asg_service_discovery_role[0].assume_role_policy) : null
  
  asg_service_discovery_role_has_rtbfabric_trust = local.asg_service_discovery_role_trust_policy != null ? (
    length([
      for stmt in local.asg_service_discovery_role_trust_policy.Statement :
      stmt if contains(keys(stmt), "Principal") && contains(keys(stmt.Principal), "Service") && 
      length(setintersection(
        toset(flatten([stmt.Principal.Service])),
        toset(local.rtbfabric_principals_required)
